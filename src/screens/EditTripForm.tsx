...
import { setTripsByUser, tripsByUser } from "../store/slices/tripSlice";
...
const EditTripForm = ({ navigation }: tripDetailsProps) => {
  ...
  const [start_date, setStartDate] = useState(
    formatDate(trip.start_date * 1000)
  );
  const [end_date, setEndDate] = useState(formatDate(trip.end_date * 1000));
  ...
  const handleSubmit = async () => {
    ...
    await tripAPI.updateTrip(trip._id, {
      organizer_id: userID,
      name: name,
    ...
    Alert.alert("Trip Updated", "The trip details have been updated", [
      {
        text: "OK",
        onPress: async () => {
          navigation.goBack();
