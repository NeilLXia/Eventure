...
const NewEventForm = ({ navigation }: newEventFormProps) => {
  const dispatch = useDispatch();
  const userID = useSelector((state: any) => state.appState.userID);
  const tripID = useSelector((state: any) => state.appState.currentTrip)._id;
  ...
  const handleSubmit = async () => {
    ...
    Alert.alert(
      "Event Posted",
      "A new event has been created! Start inviting your friends!",
      [
        {
          text: "OK",
          onPress: () => {
  ...
  return (
    <SafeAreaView className="flex-1" style={{ paddingTop }}>
      ...
      >
        <ScrollView>
          <Text className="text-2xl font-semibold mt-8">Event Name</Text>
